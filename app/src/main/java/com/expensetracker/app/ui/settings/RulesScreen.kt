            ...
            items(rules, key = { it.merchantKey }) { rule ->
                ...
                Row(
                    ...
                    Column(Modifier.weight(1f).padding(start = 12.dp)) {
                        ...
                        Text(category.displayName(), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    IconButton(onClick = { scope.launch { container.repository.deleteRule(rule) } }) {
                        Icon(Icons.Outlined.Delete, contentDescription = stringResource(R.string.rules_remove, rule.merchantName))
                    }
                }
